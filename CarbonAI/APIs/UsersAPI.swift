...
open class UsersAPI {
     ...
     */
    @discardableResult
    open class func whoAmISync(apiResponseQueue: DispatchQueue = CarbonAIAPI.apiResponseQueue, completion: @escaping ((_ data: UserResponse?, _ error: Error?) -> Void)) -> RequestTask {
     ...
     */
    @available(iOS 13.0, macOS 10.15, tvOS 13.0, watchOS 6.0, *)
    private class func whoAmIAsyncMappedParams() async throws -> UserResponse {
        return try await withCheckedThrowingContinuation { continuation in
            whoAmIWithRequestBuilder().execute { result in
                switch result {
                case let .success(response):
                    continuation.resume(returning: response.body)
                case let .failure(error):
    ...
    /**
     ...
     - returns: RequestBuilder<UserResponse> 
     */
    open class func whoAmIWithRequestBuilder(
    ) -> RequestBuilder<UserResponse> {
        let basePath = CarbonAIAPI.basePath;
        ...
        fatalError("Error: Unable to send request to GET /whoami")
    }
    ...
    /**
     ...
     - returns: RequestBuilder<UserResponse> 
     */
    open func whoAmIWithRequestBuilder(
    ) -> RequestBuilder<UserResponse> {
        let basePath = self.client!.basePath;
        let localVariablePath = "/whoami"
        let localVariableURLString = basePath + localVariablePath
